...
import { Award, Sun, Moon, Palette, Watch, Heart, Clock } from 'lucide-react';
...
export const TopHeader: React.FC<TopHeaderProps> = ({
  ...
  return (
    <header className="flex justify-between items-center px-4 md:px-6 h-16 w-full z-50 fixed top-0 bg-[var(--bg-header)] backdrop-blur-md border-b border-[var(--border-subtle)] transition-colors duration-200">
      ...
      {/* Live Clock — Center (hidden on very small) */}
      <div
        className="hidden md:flex flex-col items-center cursor-pointer group select-none"
        onClick={() => { sounds.playClick(); onOpenSmartwatchModal(); }}
        title="Abrir Centro de Reloj y Biometría"
      >
      ...
      <div className="flex items-center gap-1.5 md:gap-2">
        ...
        <button
          onClick={() => { sounds.playClick(); onOpenSmartwatchModal(); }}
          className={`px-2.5 py-1.5 rounded-full border flex items-center gap-1.5 text-xs font-mono font-bold transition-all active:scale-95 ${
          ...
          title={smartwatchData.connected ? `En vivo: ${smartwatchData.deviceName}` : 'Conectar Reloj & Biometría'}
        >
        ...
        <div 
          className="hidden lg:flex flex-col items-end cursor-pointer mr-1"
          onClick={() => handleTabClick('gamification')}
          title="Ver Desafíos y Nivel"
        >
        ...
        <div className="relative">
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-full bg-[var(--bg-input)] border border-[var(--border-subtle)] text-[var(--text-main)] hover:border-[var(--accent-color)] active:scale-95 transition-all"
          ...
          {showColorPicker && (
            <div className="absolute right-0 mt-2 p-2.5 rounded-2xl glass-card shadow-2xl border border-[var(--border-card)] z-50 flex items-center gap-2 animate-fade-in bg-[var(--bg-card-solid)]">
              {THEME_OPTIONS.map((t) => (
                <button
                  key={t.id}
